...
private:
	...
	// Mis funciones
	void construir_arbol(bintree<Pregunta> & Arb, bintree<Pregunta>::node n, int index_atributo, const vector<vector<bool> > &tableroAux, const vector<string> &personajes);
	void calcular_profundidad_promedio_hojas(const bintree<Pregunta> &a, bintree<Pregunta>::node n, float &totalProfundidad, int profundidadActual);
	void eliminar_nodos_redundantes(bintree<Pregunta>& a, bintree<Pregunta>::node n);
	void calcular_informacion_jugada(const bintree<Pregunta> &a, bintree<Pregunta>::node n, set<string> &nombres);
